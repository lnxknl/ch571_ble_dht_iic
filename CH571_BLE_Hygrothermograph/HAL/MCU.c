...
tmosEvents HAL_ProcessEvent(tmosTaskID task_id, tmosEvents events)
{
    ...
    if(events & LED_BLINK_EVENT)
    {
        ...
        DelayUs(2);
        GPIOB_ResetBits(GPIO_Pin_14);//PB14置低打开LED
        tmos_start_task(halTaskID,LED_OFF_EVENT,MS1_TO_SYSTEM_TIME(1000));//1秒后关闭LED

        return events ^ LED_BLINK_EVENT;
    }
    if(events & LED_OFF_EVENT)
    {
        GPIOB_SetBits(GPIO_Pin_14);//PB14置高关闭LED
        DelayUs(2);
        GPIOB_ModeCfg(GPIO_Pin_14, GPIO_ModeIN_PU);

        return events ^ LED_OFF_EVENT;
    }
...
#endif
    tmos_start_task( halTaskID, LED_BLINK_EVENT, 160 );//LED亮1秒
}
